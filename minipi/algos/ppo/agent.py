...
from minipi.algos.ppo.policy import PPOBasePolicy
from minipi.buffer.common import Buffer
from minipi.utils.schedulers import Schedulable
from minipi.utils.misc import calculate_gae, get_scheduler, explained_variance
...
class PPOLearner(Learner):
    ...
    def learn(self, scheduler_step: int, buffer: Union[Buffer, RRef]):
        ...
        if self.policy.is_recurrent:
            ...
            batch_size = B * T
            indices = np.mgrid[0:T, 0:B].reshape(2, batch_size).T
        ...
        for _ in range(self.n_epochs):
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                if self.policy.is_recurrent:
                    ...
                    sub_indices = (slice(None), sub_indices)
                    rnn_states = batch["rnn_states"][sub_indices]
                else:
                    sub_indices = indices[start:end]
                ...
                pg_loss, vf_loss, entropy, extra_out = self.policy.loss(
                    obs=batch["obs"][sub_indices],
                    advs=batch["adv"][sub_indices],
                    firsts=batch["first"][sub_indices],
                    actions=batch["action"][sub_indices],
                    old_values=batch["value"][sub_indices],
                    old_logpacs=batch["logpac"][sub_indices],
